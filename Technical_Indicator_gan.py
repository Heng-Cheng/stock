...
def timesteps(dataframe, days = 20):
    ...
    features = [column for column in dataframe.columns if column != 'date']
...
def output_data():
    ...
    for shares in sorted(os.listdir('./stock_clean/')):
        if not shares.startswith('.'):
            ...
            timesteps(df, days=8)
            x_train = df.mask(df.astype(object).eq('None')).dropna().reset_index(drop=True)

            features = ['date', 'open', 'high', 'low', 'close', 'high_change', 'low_change', 'close_change']

            x_train_chosen = df[features]
            timesteps(x_train_chosen, days=8)
            x_train_chosen = x_train_chosen.mask(df.astype(object).eq('None')).dropna().reset_index(drop=True)

            g_time = 1
            y_train = x_train[features]
            y_train = y_train.shift(periods=-g_time)
            y_train = y_train.mask(y_train.astype(object).eq('None')).dropna().reset_index(drop=True)

            os.mkdir("./gan/{}".format(dirname))
            x_train.round(4).iloc[:-g_time].to_csv('./gan/{}/x_train.csv'.format(dirname), index=False)
            x_train_chosen.round(4).iloc[:-g_time].to_csv('./gan/{}/x_train_chosen.csv'.format(dirname), index=False)
            y_train.round(4).to_csv('./gan/{}/y_train.csv'.format(dirname), index=False)
    ...
    pd.DataFrame(Z_data).to_csv('./gan/Z_data.csv', index=False)
